...
class Food(pygame.sprite.Sprite):
    ...
    def __init__(self, colour, size, effect, position):
        ...
        self.properties['time']   = 4 * Config.FPS # -1 = infinite
...
class FoodCurse(Food):
    _DEFAULT_COLOUR = Config.BACKGROUND_COLOUR
    _DEFAULT_SIZE   = [20, 20]
...
def make_food(snake, foodType='FoodNormal', colour=None, size=None, effect=None):
    ...
    hbound = Config.DEFAULT_SCREEN_SIZE[0] / (foodTypeX - 1)
    vbound = Config.DEFAULT_SCREEN_SIZE[1] /  (foodTypeY- 1)
    (X, Y) = (None, None)
